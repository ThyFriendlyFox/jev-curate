...
Run the implementation phase as a loop with exactly this shape — a short
prompt, with all intelligence living in the kit files (this is the
pattern that works; do not fatten the prompt):
...
**0. Preflight.** Fetch; branch from latest `master` as
...
**3. Build.** The loop: implement → `./verify/verify.sh` → commit at each
