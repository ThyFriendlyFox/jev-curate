...
|---|---|---|
| First time this kit is in this repo (double-brace placeholders still exist anywhere in the kit) | `SETUP.md` | One-time tailoring. Do this before anything else. |
...
| Touching the Jev client seam (live/mock) | `docs/CLIENTS.md` | Includes the new-client contract. |
...
| Asked for a blog post / public write-up of recent work | `.claude/skills/blog-post/SKILL.md` | Devlog in, post out. Never publishes without a go-ahead. |
| Asked to "make the README true", or a README claim has no proof | `../ralph/GOAL.md` + `../ralph/loop.sh` | The Ralph loop: one fixed prompt until `./verify/verify.sh` is green. |
...
- Commit at boundaries with evidence in the message. Never commit directly to `master`.
